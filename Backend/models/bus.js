...
const getBusNum = (id) => {
  return new Promise((resolve, reject) => {
    if (id) {
      const query =
        "SELECT `bus_number` FROM `bus_info` WHERE `place_id`=? GROUP BY `bus_number`";
...
const getBusPlaces = (busNum) => {
  return new Promise((resolve, reject) => {
    const query =
      "SELECT GROUP_CONCAT(`place_id`)as 'placesId' FROM `bus_info` WHERE `bus_number`=? GROUP BY `bus_number`";
